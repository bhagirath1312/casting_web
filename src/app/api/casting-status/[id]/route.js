...
import CastingInterest from '@/models/InterestSubmission'; // Import the model

// GET a single casting post or fetch applicants based on query parameter
export async function GET(req, { params }) {
  await connectDB();
  ...
  try {
    if (showInterests === "true") {
      // Fetch applicants for the casting post
      const users = await CastingInterest.find({ castingId: params.id }).lean();
      return NextResponse.json({ success: true, users }, { status: 200 });
    }

    // Fetch the casting post itself
    const casting = await Casting.findById(params.id);
    if (!casting) {
      return NextResponse.json({ message: 'Casting post not found' }, { status: 404 });
    }
...
>>>>>>> ad63770 (imporve homepage and admin dashbord)
...
export async function PATCH(req, { params }) {
  ...
  try {
    const { id } = params;
    const data = await req.json();
    const updated = await Casting.findByIdAndUpdate(id, data, { new: true });
...
export async function DELETE(req, { params }) {
  ...
  try {
    const { id } = params;
    const deleted = await Casting.findByIdAndDelete(id);
    if (!deleted) {
      return NextResponse.json({ message: 'Casting post not found' }, { status: 404 });
    }
